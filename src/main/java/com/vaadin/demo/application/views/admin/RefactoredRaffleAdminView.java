...
/**
 * Refactored RaffleAdminView that uses domain records instead of JPA entities
 */
@Menu(order = 5, icon = LineAwesomeIconUrl.TOOLS_SOLID, title = "Raffle Admin")
@AnonymousAllowed
@PageTitle("Raffle Admin")
@Route("raffle-admin-new")
@SuppressWarnings("serial")
public class RefactoredRaffleAdminView extends VerticalLayout {
    ...
    public RefactoredRaffleAdminView(
        ...
        raffleGrid.addColumn(RaffleRecord::meetupEventId).setHeader("Meetup Event ID");
        raffleGrid.addColumn(raffle -> {
            if (raffle.event() != null) {
                return raffle.event().title();
            } else {
                try {
                    return meetupService.getEvent(raffle.meetupEventId()).get().title();
                } catch (Exception e) {
    ...
    private void addButtonClicked(ClickEvent<Button> buttonClickEvent) {
        ...
        List<String> existingRaffleMeetupIds = raffleService.getAllRaffles().stream()
                .map(RaffleRecord::meetupEventId)
                .toList();
